 ...
 * @author carluchocp
 */
public class SRT implements AlgoritmoPlanificacion {
    ...
    public boolean debePreemptar(Proceso enEjecucion, Cola<Proceso> colaListos) {
        ...
        int tiempoActual = enEjecucion.getInstruccionesTotales() - enEjecucion.getInstruccionesEjecutadas();

        Nodo<Proceso> actual = colaListos.getPrimerNodo();
        while (actual != null) {
            Proceso p = actual.getContenido();
            int restante = p.getInstruccionesTotales() - p.getInstruccionesEjecutadas();
            if (restante < tiempoActual) {
                return true;
            }
            actual = actual.getSiguiente();
        }
    ...
    public PoliticaPlanificacion getPolitica() {
        return PoliticaPlanificacion.SRT;
    }
}
